...
cardColumns = ['ID', 'Name', 'Rating', 'Position', 'Revision', 'Price | PS', 'WeakFoot',
               'Skill Moves', 'Pace', 'Shooting', 'Passing', 'Dribbling', 'Defending',
               'Phyiscality', 'Body Type', 'Weight', 'Popularity', 'BaseStats', 'InGameStats',
               'WorkRate', 'Height', 'Country', 'Club',
               'League']

C = open('FutBinCards22.csv', 'w')
...
for key, value in fifa.items():
    ...
    for page in range(1, 2):  # int(TotalPages) + 1):
        ...
        df.to_csv('FutBinCards22.csv', mode='a', header=False,
